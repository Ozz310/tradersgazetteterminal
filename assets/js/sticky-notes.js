...
        // REMOVED: `let userId = 'single_user_id';`
        
        // --- Loader Functions ---
        function showLoader() {
            if (loaderOverlay) {
...
            const userId = getUserId(); // UPDATED: Get the userId from local storage
            if (!userId) {
                console.error('User ID not found in local storage. Cannot fetch notes.');
                hideLoader();
                return;
            }
...
            try {
                // UPDATED: Use a GET request with query parameters and the correct userId
...
            isSaving = true;
            ...
            const userId = getUserId(); // UPDATED: Get the userId from local storage
            if (!userId) {
                console.error('User ID not found in local storage. Cannot save notes.');
                isSaving = false;
                return;
            }

            try {
                // UPDATED: Use a POST request with a JSON body and the correct userId
...
        }
        
        // ... rest of your code to handle UI events and note rendering ...
