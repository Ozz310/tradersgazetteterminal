...
                        // Using stopVideo is better than pause for a clean end state
                        player.stopVideo(); 
                    }
                }
            }
...
        // We need a slight delay to ensure the DOM has rendered the video-placeholder div
        // before the YouTube API tries to create the player inside it.
        setTimeout(() => {
            if (window.YT && window.YT.Player) {
                createYouTubePlayer(book.videoId); 
            } else {
                console.warn('YouTube API not fully ready, delaying player creation.');
            }
        }, 100);
    }
...
    function closeModal() {
        if (player) {
            player.destroy(); // 🚀 FIX: Use destroy() instead of stopVideo() for cleanup
            player = null;
        }
        modal.classList.remove('open');
        // Optional: Clean up modal content to prevent flash of old data
